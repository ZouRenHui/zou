...
<#
.SYNOPSIS
  在 Windows 上一键构建 SpeechToolbox.exe、免安装 zip 与安装包 SpeechToolboxSetup.exe
...
$DistDir = Join-Path $ProjectRoot "dist\SpeechToolbox"
$InstallerOut = Join-Path $ProjectRoot "installer\output"
$SpecFile = Join-Path $BuildDir "speech_toolbox.spec"
$PortableZip = Join-Path $InstallerOut "SpeechToolbox-Portable.zip"
...
Write-Host ""
Write-Host "正在打包 SpeechToolbox.exe（可能需要较长时间）..."
Push-Location $ProjectRoot
try {
...
$MainExe = Join-Path $DistDir "SpeechToolbox.exe"
...
$SetupExe = Join-Path $InstallerOut "SpeechToolboxSetup.exe"
